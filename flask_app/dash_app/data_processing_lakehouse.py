...
# If in a terminal, not exactly sure how it will work.
...
def process_COPPER():
    ...
    ironsmith_undata(paths.UNDATA_PATH_COPPER, paths.UNDATA_PATH_IRON)
...
def update_config(meta_path, config_path):
    ...
    config = config.drop_duplicates(subset=['dataset_raw'])
...
def smelt_iron(origin, meta_path, stats_path):
    ...
    #testing
    #origin = "/data_lakehouse/iron/statistics/" #smelting
    #meta_path = "/data_lakehouse/titanium/meta/master_meta.parquet" 
    #stats_path = "/data_lakehouse/titanium/statistics/master_stats.parquet" 
    ...
    print("Purging rows dataset-year combinations with less than 100 country datapoints...")
    master = master.groupby(['dataset_raw', 'year'], as_index=False).apply(lambda x: x if len(x) > 100 else pd.DataFrame())      
...
def ironsmith_gapminder_world_dev_indicators(origin, destination): 
    ...
    lookup = pd.read_parquet(metapath1).fillna("Not available")     
